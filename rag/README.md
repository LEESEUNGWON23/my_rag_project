...
### 3-5. PDF 이미지 처리: 수집 blocking → background task 분리

**Situation**  
PDF에서 이미지를 추출한 뒤 스토리지에 업로드하고 URL을 payload에 연결하는 작업이 파일 수집 요청 안에서 동기로 실행되고 있었습니다. 이미지가 많은 PDF일수록 업로드 API 응답이 느려졌고, 업로드 중 오류가 나면 수집 전체가 실패했습니다.

**Action**  
URL linking 작업만 background task로 분리했습니다. 수집 pipeline은 이미지 추출까지만 완료하고 즉시 응답을 반환하며, URL 연결은 별도로 처리됩니다.

```python
# Before: 이미지 업로드·URL 연결이 수집 요청 안에서 blocking 실행
upload_res = upload_to_spring(file.id, image_files)
for s in upload_res.successes:
    url_map[s.filename] = build_image_layout_url(file.id, s.fileId)
# ... URL을 payload에 연결 후 응답

# After: URL linking만 background task로 분리
async def _link_image_urls_in_background(file_id, image_files, docs):
    upload_res = upload_to_spring(file_id, image_files)
    # ... URL 연결 처리

background_tasks.add_task(_link_image_urls_in_background, file.id, image_files, docs)
# 수집은 즉시 완료 응답 반환
```

**Result**  
이미지가 많은 PDF에서도 수집 API 응답 시간이 단축되었고, 이미지 업로드 실패가 수집 전체에 영향을 주지 않게 되었습니다.

---

### 3-6. 알려진 한계: 특정 폰트 유니코드 깨짐
...
- **MMR Selector 성능 개선:** 검색 결과 다양성 확보를 위한 MMR Selector에서 개별 encode 호출을 배치 1회 호출로 변경해 불필요한 API 왕복 제거
- **Deep Research API:** 사용자 query로 웹을 검색·스크래핑하고 결과를 기존 RAG pipeline에 통합하는 엔드포인트 구현 (DuckDuckGo 검색 + Trafilatura 병렬 스크래핑)
- **prd / dev collection 분리:** Qdrant collection을 환경별로 분리해 개발·운영 데이터 격리
- **파일 버전 관리:** 문서 재업로드 시 기존 버전 자동 삭제 및 재embedding 로직 구현
